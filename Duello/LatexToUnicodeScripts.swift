import Foundation

/// Conversion des exposants et indices en caractères Unicode : les tables
/// `subscripts`/`superscripts` et le rendu `enScript` qui les applique. Élargis
/// de `private` à `internal` parce que lus par `LatexToUnicodeRendering` —
/// aucun membre n'est renommé.
extension LatexToUnicode {
    // MARK: Exposants et indices

    /// Exposants et indices disponibles en Unicode (`mathLayout.ts`).
    static let subscripts: [Character: String] = [
        "0": "₀", "1": "₁", "2": "₂", "3": "₃", "4": "₄",
        "5": "₅", "6": "₆", "7": "₇", "8": "₈", "9": "₉",
        "+": "₊", "-": "₋", "−": "₋", "=": "₌", "(": "₍", ")": "₎",
        "a": "ₐ", "e": "ₑ", "h": "ₕ", "i": "ᵢ", "j": "ⱼ", "k": "ₖ", "l": "ₗ", "m": "ₘ",
        "n": "ₙ", "o": "ₒ", "p": "ₚ", "r": "ᵣ", "s": "ₛ", "t": "ₜ", "u": "ᵤ", "v": "ᵥ", "x": "ₓ",
    ]

    static let superscripts: [Character: String] = [
        "0": "⁰", "1": "¹", "2": "²", "3": "³", "4": "⁴",
        "5": "⁵", "6": "⁶", "7": "⁷", "8": "⁸", "9": "⁹",
        "+": "⁺", "-": "⁻", "−": "⁻", "=": "⁼", "(": "⁽", ")": "⁾",
        "a": "ᵃ", "b": "ᵇ", "c": "ᶜ", "d": "ᵈ", "e": "ᵉ", "f": "ᶠ", "g": "ᵍ", "h": "ʰ", "i": "ⁱ",
        "j": "ʲ", "k": "ᵏ", "l": "ˡ", "m": "ᵐ", "n": "ⁿ", "o": "ᵒ", "p": "ᵖ", "r": "ʳ", "s": "ˢ",
        "t": "ᵗ", "u": "ᵘ", "v": "ᵛ", "w": "ʷ", "x": "ˣ", "y": "ʸ", "z": "ᶻ",
        "T": "ᵀ", "N": "ᴺ",
    ]

    /// Rend en exposant ou en indice, ou `nil` si un caractère résiste.
    static func enScript(_ texte: String, _ table: [Character: String]) -> String? {
        if texte.trimmingCharacters(in: .whitespaces).isEmpty { return nil }
        var rendu = ""
        for caractere in texte {
            if estEspace(caractere) { continue }
            guard let equivalent = table[caractere] else { return nil }
            rendu += equivalent
        }
        return rendu
    }
}
