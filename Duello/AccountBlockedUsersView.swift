//
//  AccountBlockedUsersView.swift
//  Duello
//
//  Écran « Comptes bloqués » : port fidèle de
//  `expo_ref/src/screens/BlockedUsersScreen.tsx`.
//
//  La version précédente était une démo locale (liste figée, retrait
//  immédiat, sans réseau). Elle est remplacée par le port réseau : chargement
//  via `fetchUserSafetyState` (`ReportSafetyAPI.fetchState`), déblocage via
//  `unblockSocialProfile` (`ReportSafetyAPI.unblock`), états
//  chargement/erreur/Réessayer, confirmation avant déblocage, chapeau
//  « SÉCURITÉ » + titre, avatar avec photo et pastille de présence, chevron de
//  retour en tête. Mêmes règles de style que la source : carte d'explication
//  fond `surfaceMuted` sans bordure (padding 15) et carte de membre
//  padding 11 / minHeight 66.
//
//  Limite : SwiftUI n'expose pas de « live region » ; l'erreur en ligne est
//  annoncée via `UIAccessibility.post(notification: .announcement, ...)`,
//  équivalent du `accessibilityLiveRegion="polite"` de React Native.
//
import SwiftUI
#if canImport(UIKit)
import UIKit
